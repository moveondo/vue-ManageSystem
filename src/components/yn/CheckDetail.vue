           ...
           handleSizeChange(val) {
            ...
            queryPersonData(listingId){
                   ...
                   for(let i=0; i< Length; i++){
                    ...
                    if(self.UserMessage[0].ktpSelfPhotoUrl){
            ...
            Verify(index,row){
               ...
               if(index==0){
                ...
                if(index==1){
                   ...
                   self.$axios.post(self.VerifyBankurl, {"id":id,"userId":userId,"bankCardNo":bankCardNo,"bankName":bankName,"bankCode":bankCode,"ownerName":ownerName,"receptionHolderName":receptionHolderName,"auditStatus":0}).then((res) => {
                        if(res.data.result==0){
                        this.$message.success('通过 (terlewati)');
                        }
                          this.Reload();
             ...
             Refuse(index,row){
              ...
              if(index==0){
                   ...
                   self.$axios.post(self.VerifyBankurl, {"id":id,"userId":userId,"bankCardNo":bankCardNo,"bankName":bankName,"bankCode":bankCode,"ownerName":ownerName,"receptionHolderName":receptionHolderName,"auditStatus":-1}).then((res) => {
                        if(res.data.result==0){
                        this.$message.success('拒绝(tolak)');
                        }else if( res.data.result=='-99' || res.data.result=='-999'){
                            self.$message.error(res.data.resultMessage);
